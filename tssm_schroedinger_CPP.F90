...
module S(tssmq_schroedinger) ! (Nonlinear) Schroedinger
    ...
    use tssmq_generalized_laguerre
#else
    use tssmq_fourier
#endif
#else
...
#elif defined(_LAGUERRE_)
    type, extends(SB(generalized_laguerre)) :: S(schroedinger)
#elif defined(_ROTATING_)
    type, extends(SB(fourier_rotating)) :: S(schroedinger)
#else
...
!        real(kind=prec) :: Omega = 0.0_prec 
...
#elif defined(_LAGUERRE_)
    type, extends(SB(wf_generalized_laguerre)) :: S(wf_schroedinger)
#elif defined(_ROTATING_)
    type, extends(SB(wf_fourier_rotating)) :: S(wf_schroedinger)
#else
    ...
    contains
        ...
        procedure :: imaginary_time_propagate_C
...
contains
    ...
    function new_method( &
        ...
        !TODO: scaling of Omega w.r.t. hbar, mass!!!
#if(_DIM_==2)
        this%SB(fourier_rotating) = SB(fourier_rotating)(nx, xmin, xmax, ny, ymin, ymax, Omega)
#elif(_DIM_==3)
        this%SB(fourier_rotating) = SB(fourier_rotating)(nx, xmin, xmax, ny, ymin, ymax, nz, zmin, zmax, Omega)
#endif
#else 
#if(_DIM_==1)
...
#elif defined(_LAGUERRE_)
        call this%SB(generalized_laguerre)%finalize()
#elif defined(_ROTATING_)
        call this%SB(fourier_rotating)%finalize()
#else
...
! TODO: nondefault values for hbar, omega
#ifdef _REAL_
        this%SB(wf_fourier_rotating) = SB(wf_fourier_rotating)(m, coefficient=m%hbar/(2.0_prec*m%mass))
#else
        this%SB(wf_fourier_rotating) = SB(wf_fourier_rotating)(m, &
                                          coefficient=cmplx(0.0_prec, m%hbar/(2.0_prec*m%mass), kind=prec))
#endif
#else
#ifdef _REAL_
...
#elif defined(_LAGUERRE_)
!TODO write grid attributes for laguerre grid
#else
...
!TODO
    ...
    subroutine imaginary_time_propagate_C(this, dt)
        class(S(wf_schroedinger)), intent(inout) :: this
        _COMPLEX_OR_REAL_(kind=prec), intent(in) :: dt
#ifdef _REAL_
        call this%propagate_C(dt) 
#else
        call this%propagate_C(dt*cmplx(0.0_prec,-1.0_prec, kind=prec)) 
#endif
    end subroutine imaginary_time_propagate_C
...
!$OMP PARALLEL DO PRIVATE(j, uf, ev) REDUCTION(+:h)
        do j=1,n_threads
            uf => this%uf(lbound(this%uf,1)+m%jf(j-1):lbound(this%uf,1)+m%jf(j)-1)
...
#if(_DIM_==1)
#ifndef _OPENMP
        h = 2.0_prec*sum(m%eigenvalues1 * (real(this%ufc,prec)**2 + aimag(this%ufc)**2) ) 
#else
        h = 0.0_prec
!$OMP PARALLEL DO PRIVATE(j, ufc, ev) REDUCTION(+:h)
        do j=1,n_threads
            ufc => this%ufc(lbound(this%ufc,1)+m%jf(j-1):lbound(this%ufc,1)+m%jf(j)-1)
            ev => m%eigenvalues1(lbound(m%eigenvalues1,1)+m%jf(j-1):&
                                         lbound(m%eigenvalues1,1)+m%jf(j)-1)
...
!$OMP PARALLEL DO PRIVATE(j, ufc, ev) REDUCTION(+:h)
        do j=1,n_threads
            ufc => this%ufc(:,lbound(this%ufc,2)+m%jf(j-1):lbound(this%ufc,2)+m%jf(j)-1)
            ev => m%eigenvalues2(lbound(m%eigenvalues2,1)+m%jf(j-1):&
                                         lbound(m%eigenvalues2,1)+m%jf(j)-1)
...
!$OMP PARALLEL DO PRIVATE(j, ufc, ev) REDUCTION(+:h)
        do j=1,n_threads
            ufc => this%ufc(:,:,lbound(this%ufc,3)+m%jf(j-1):lbound(this%ufc,3)+m%jf(j)-1)
            ev => m%eigenvalues3(lbound(m%eigenvalues3,1)+m%jf(j-1):&
                                         lbound(m%eigenvalues3,1)+m%jf(j)-1)
            h = h +  2.0_prec &
...
#endif        
#if defined(_LAGUERRE_)
        real(kind=prec), pointer :: nodes_x(:,:)
        real(kind=prec), pointer :: nodes_y(:,:)
#endif        
        real(kind=prec), pointer :: nodes(:)
        integer :: j
#endif   
...
!$OMP END PARALLEL DO
          ...
          *spread(spread(m%g%weights_z*m%g%nodes_z, 1, m%g%n1max-m%g%n1min+1), 3, m%g%n3max-m%g%n3min+1) &
          ...
          x_mean = 0.0_prec
          y_mean = 0.0_prec
!$OMP PARALLEL DO PRIVATE(j, u, nodes_x, nodes_y) REDUCTION(+:x_mean,y_mean)
          do j=1,n_threads
#if(_DIM_==2)
                u => this%u(:,lbound(this%u,2)+m%g%jj(j-1):lbound(this%u,2)+m%g%jj(j)-1)
#elif(_DIM_==3)
...
#if(_DIM_==3)
          z_mean = 0.0_prec
!$OMP PARALLEL DO PRIVATE(j, u) REDUCTION(+:z_mean)
          do j=1,n_threads
                u => this%u(:,:,lbound(this%u,3)+m%g%jj(j-1):lbound(this%u,3)+m%g%jj(j)-1)
                z_mean =  z_mean + sum(spread(spread(m%g%weights_r, 2, m%g%n2max-m%g%n2min+1), &
                   3, m%g%jj(j)-m%g%jj(j-1) ) &
                     *spread(spread(m%g%weights_z*m%g%nodes_z, 1, m%g%n1max-m%g%n1min+1), &
                   3, m%g%jj(j)-m%g%jj(j-1) ) &
#ifdef _REAL_
...
!$OMP END PARALLEL DO
          ...
          *spread(spread(m%g%weights_z*(m%g%nodes_z-z_mean)**2, 1, m%g%n1max-m%g%n1min+1), 3, m%g%n3max-m%g%n3min+1) & 
          ...
          x_dev = 0.0_prec
          y_dev = 0.0_prec
!$OMP PARALLEL DO PRIVATE(j, u, nodes_x, nodes_y) REDUCTION(+:x_dev,y_dev)
          do j=1,n_threads
#if(_DIM_==2)
                u => this%u(:,lbound(this%u,2)+m%g%jj(j-1):lbound(this%u,2)+m%g%jj(j)-1)
#elif(_DIM_==3)
...
!$OMP PARALLEL DO PRIVATE(j, u) REDUCTION(+:z_mean)
          do j=1,n_threads
                u => this%u(:,:,lbound(this%u,3)+m%g%jj(j-1):lbound(this%u,3)+m%g%jj(j)-1)
                z_dev =  z_dev + sum(spread(spread(m%g%weights_r, 2, m%g%n2max-m%g%n2min+1), &
                   3, m%g%jj(j)-m%g%jj(j-1) ) &
                     *spread(spread(m%g%weights_z*(m%g%nodes_z-z_mean)**2, 1, m%g%n1max-m%g%n1min+1), &
                   3, m%g%jj(j)-m%g%jj(j-1) ) &
#ifdef _REAL_
...
!$OMP END PARALLEL DO
    ...
    function observable(this, f) result(obs)
        ...
        n1 = m%g%n1min
        o1 = lbound(this%u, 1) - n1 
#if(_DIM_>=2)
        n2 = m%g%n2min
        o2 = lbound(this%u, 2) - n2 
#endif
#if(_DIM_>=3)
        n3 = m%g%n3min
        o3 = lbound(this%u, 3) - n3 
#endif
        ...
        obs = 0.0_prec
#if(_DIM_==1)
!$OMP PARALLEL DO PRIVATE(i1) REDUCTION(+:obs)
        do i1 = n1+o1,m%g%n1max+o1
            obs = obs + f(m%g%nodes_x(i1-o1))*m%g%weights_x(i1-o1) &
#ifdef _REAL_
                   *this%u(i1)**2
#else
                   *(real(this%u(i1),prec)**2+aimag(this%u(i1))**2)
#endif
        end do
!$OMP END PARALLEL DO 
#elif(_DIM_==2)       
        ...
        do i2 = n2+o2, m%g%n2max+o2
            do i1 = n1+o1,m%g%n1max+o1
                obs = obs + f(m%g%nodes_x(i1-o1),m%g%nodes_y(i2-o2)) &
                      ...
                      *this%u(i1,i2)**2
#else
                      *(real(this%u(i1,i2),prec)**2+aimag(this%u(i1,i2))**2)
#endif
            end do
        end do     
!$OMP END PARALLEL DO 
#elif(_DIM_==3)
!$OMP PARALLEL DO PRIVATE(i1, i2, i3) REDUCTION(+:obs)
        do i3 = n3+o3, m%g%n3max+o3
           do i2 = n2+o2, m%g%n2max+o2
                do i1 = n1+o1,m%g%n1max+o1
                    obs = obs + f(m%g%nodes_x(i1-o1),m%g%nodes_y(i2-o2),m%g%nodes_z(i3-o3)) &
                          *m%g%weights_x(i1-o1)*m%g%weights_y(i2-o2)*m%g%weights_z(i3-o3) &
#ifdef _REAL_
                          *this%u(i1,i2,i3)**2
#else
...
!$OMP END PARALLEL DO 
        ...
        obs = 0.0_prec
#if(_DIM_==1)
!$OMP PARALLEL DO PRIVATE(i1, x) REDUCTION(+:obs)
        do i1 = n1+o1,m%g%n1max+o1
            x = m%g%xmin + m%g%dx*(i1-o1)    
#ifdef _REAL_
            obs = obs + f(x)*this%u(i1)**2
#else
            obs = obs + f(x)*(real(this%u(i1),prec)**2+aimag(this%u(i1))**2)
#endif
        end do
!$OMP END PARALLEL DO 
        obs = obs * m%g%dx 
...
!$OMP PARALLEL DO PRIVATE(i1, x, i2, y) REDUCTION(+:obs)
        do i2 = n2+o2, m%g%n2max+o2
            ...
            do i1 = n1+o1,m%g%n1max+o1
                x = m%g%xmin + m%g%dx*(i1-o1)    
#ifdef _REAL_
                obs = obs + f(x,y)*this%u(i1,i2)**2
#else
                obs = obs + f(x,y)*(real(this%u(i1,i2),prec)**2+aimag(this%u(i1,i2))**2)
#endif
            end do
        end do     
!$OMP END PARALLEL DO 
...
#elif(_DIM_==3)
!$OMP PARALLEL DO PRIVATE(i1, x, i2, y, i3, z) REDUCTION(+:obs)
        do i3 = n3+o3, m%g%n3max+o3
           ...
           do i2 = n2+o2, m%g%n2max+o2
                ...
                do i1 = n1+o1,m%g%n1max+o1
                    x = m%g%xmin + m%g%dx*(i1-o1)    
#ifdef _REAL_
                    obs = obs + f(x,y,z)*this%u(i1,i2,i3)**2
#else
                    obs = obs + f(x,y,z)*(real(this%u(i1,i2,i3),prec)**2+aimag(this%u(i1,i2,i3))**2)
#endif
                end do
           end do     
        end do    
